    ...
    <div class="relative flex items-center justify-between h-16">
        ...
        <div class="hidden lg:block lg:ml-6">
          <div class="flex space-x-4">
            ...
            <a href={PostsAction} class="text-gray-300 hover:bg-gray-700 hover:text-white
                px-3 py-2 rounded-md text-sm font-medium">Posts</a>
            <a href={UsersAction} class="text-gray-300 hover:bg-gray-700 hover:text-white
                px-3 py-2 rounded-md text-sm font-medium">Users</a>
          </div>
        </div>
      </div>
      ...
      <div class="flex lg:hidden">
        ...
        <button x-on:click="mobileMenu = !mobileMenu" class="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-white hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white" aria-expanded="false">
          ...
          -->
          <svg x-bind:class="{'hidden': mobileMenu, 'block': !mobileMenu}" class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
          ...
          -->
          <svg x-cloak x-bind:class="{'block': mobileMenu, 'hidden': !mobileMenu}" class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
...
profileHtml user = [hsx|
      <div class="hidden lg:block lg:ml-4">
        <div class="flex items-center">
          ...
          <!-- Profile dropdown -->
          <div x-data="{ profileDropdown: false }" class="ml-4 relative flex-shrink-0">
            <div>
              <button 
                  x-on:click="profileDropdown = !profileDropdown"
                  x-on:click.away="profileDropdown = false"
                  class="bg-gray-800 rounded-full flex text-sm text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" id="user-menu" aria-haspopup="true">
...
adminLinks :: Admin -> Html
adminLinks user = [hsx|
      <a href={ShowAdminAction (get #id user)} class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">Your Profile</a>
      <a href={EditAdminAction (get #id user)} class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">Settings</a>
      <a href={DeleteSessionAction} class="js-delete js-delete-no-confirm block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">Sign out</a>
|]

adminMobileLinks :: Admin -> Html
adminMobileLinks user = [hsx|
        <a href={DeleteSessionAction}>
        <div class="ml-3">
          <div class="text-base font-medium text-white">Logout</div>
          <div class="text-sm font-medium text-gray-400">{get #email user}</div>
        </div>
        </a>
    |]
